...
def export_monthly_sales(request):
    ...
    ).select_related("item__product", "agent", "location", "item__business").order_by("sold_at")
...
def export_monthly_summary(request):
    ...
    if not business:
        ...
        w.writerow([
            ...
            "gross_profit", "net_profit", "cash_amount", "bank_amount", "mobile_amount"
        ])
        return resp
    ...
    for sale in sales_qs.annotate(date=TruncDate("sold_at")).values("date").annotate(
    ...
    for cost in costs_qs.values("effective_date").annotate(
