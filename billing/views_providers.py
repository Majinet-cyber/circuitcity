...
def stripe_checkout(request: HttpRequest) -> HttpResponse:
    ...
    if not stripe_service.is_stripe_configured():
        messages.error(request, "Stripe is not configured. Please use another payment method or contact support.")
        return redirect("billing:subscribe")
    ...
    logger.info(f"Redirecting business {business.id} to Stripe checkout for plan {plan_code}")
...
def pesapal_checkout(request: HttpRequest) -> HttpResponse:
    ...
    if not pesapal_service.is_pesapal_configured():
        messages.error(request, "Pesapal is not configured. Please use another payment method or contact support.")
        return redirect("billing:subscribe")
...
def pesapal_ipn(request: HttpRequest) -> HttpResponse:
    ...
    logger.info(
        f"Pesapal IPN received: tracking_id={tracking_id}, " f"merchant_ref={merchant_ref}, type={notification_type}"
    )
