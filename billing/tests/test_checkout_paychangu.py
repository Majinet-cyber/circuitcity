...
class TestCheckoutPayChangu:
    ...
    @patch("billing.paychangu_service.create_checkout")
    def test_checkout_card_creates_paychangu_transaction(self, mock_create_checkout, setup_data):
...
class TestWebhookActivatesSubscription:
    ...
    def test_webhook_success_activates_subscription(self, mock_verify, setup_data):
        ...
        signature = hmac.new(b"test-webhook-secret", payload_bytes, hashlib.sha256).hexdigest()
    ...
    def test_webhook_idempotency_no_double_activation(self, mock_verify, setup_data):
        ...
        # First webhook call
        response1 = client.post(url, data=payload_bytes, content_type="application/json", HTTP_SIGNATURE=signature)
        ...
        response2 = client.post(url, data=payload_bytes, content_type="application/json", HTTP_SIGNATURE=signature)
...
class TestPayChanguCorrectEndpoint:
    ...
    def test_create_checkout_calls_correct_endpoint(self, mock_requests):
        ...
        mock_response.json.return_value = {"data": {"checkout_url": "https://checkout.paychangu.com/pay/test123"}}
