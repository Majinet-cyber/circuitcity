...
def hq_user(db):
    ...
    user = User.objects.create_user(username="hq_staff", email="hq@test.com", password="testpass123", is_staff=True)
    return user
...
def manager_user(db):
    ...
    return User.objects.create_user(username="manager", email="manager@test.com", password="testpass123")
...
def test_business(db):
    ...
    return Business.objects.create(name="Test Business", slug=unique_slug("Test Business"), status="ACTIVE")
...
class TestHQTicketsPage:
    ...
    def test_hq_tickets_page_shows_ticket(self, client: Client, hq_user, test_ticket):
    ...
    def test_hq_ticket_detail_loads(self, client: Client, hq_user, test_ticket):
        """HQ staff should be able to view ticket details."""
        client.force_login(hq_user)

        response = client.get(reverse("support:hq_ticket_detail", kwargs={"pk": test_ticket.pk}))
...
class TestManagerTickets:
    ...
    def test_manager_ticket_list_requires_business(self, client: Client, manager_user):
    ...
    def test_manager_can_view_their_tickets(self, client: Client, manager_user, test_business, test_ticket):
        """Manager should see only their business's tickets."""
        # Create membership
        Membership.objects.create(user=manager_user, business=test_business, role="MANAGER", status="ACTIVE")
