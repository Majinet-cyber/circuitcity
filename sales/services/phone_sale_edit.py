...
class PhoneSaleEditService:
        ...
        *,
        ...
        try:
            ...
            sale = Sale.objects.select_for_update().select_related(
                'item', 'item__product', 'agent', 'location'
            ).get(pk=sale_id)
            ...
            if new_imei is not None:
                ...
                if new_imei:
                    ...
                    conflicting_item = InventoryItem.objects.filter(
                        business=business,
                        imei=imei_digits,
                        status="SOLD"
                    ).exclude(pk=sale.item_id).first()
                    ...
                    in_stock_conflict = InventoryItem.objects.filter(
                        business=business,
                        imei=imei_digits,
                        status="IN_STOCK"
                    ).exclude(pk=sale.item_id).first()
    ...
    def _update_wallet_commission(sale: Sale, business: Business, old_price: Decimal) -> None:
        ...
        try:
            ...
            logger.info(
                f"Updated wallet commission for Sale #{sale.pk}: "
