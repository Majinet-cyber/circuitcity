...
class RollbackPermissionsTestCase(TestCase):
    ...
    def test_manager_can_rollback_sale_created_by_agent(self):
        ...
        can_rollback, error_msg = RollbackService.can_rollback(
            self.sale, self.manager1, self.business1
        )
    ...
    def test_agent_cannot_rollback_own_sale(self):
        ...
        can_rollback, error_msg = RollbackService.can_rollback(
            self.sale, self.agent1, self.business1
        )
    ...
    def test_manager_from_different_business_cannot_rollback(self):
        ...
        can_rollback, error_msg = RollbackService.can_rollback(
            self.sale, self.manager2, self.business1
        )
    ...
    def test_superuser_can_rollback(self):
        ...
        can_rollback, error_msg = RollbackService.can_rollback(
            self.sale, self.superuser, self.business1
        )
    ...
    def test_rollback_post_manager_success(self):
        ...
        if messages:
            ...
            self.assertTrue(error_found, f"Should show 'already rolled back' error. Messages: {[str(m) for m in messages]}")
    ...
    def test_rollback_post_agent_forbidden(self):
        ...
        if messages:
            error_found = any('only managers' in str(m).lower() or 'cannot rollback' in str(m).lower() 
                            for m in messages)
    ...
    def test_already_rolled_back_sale_cannot_be_rolled_back_again(self):
        ...
        self.item.refresh_from_db()
        self.assertEqual(self.item.status, after_first_rollback_item_status, 
                        "Item status should not change on second rollback attempt")
        self.assertEqual(self.item.sold_at, after_first_rollback_item_sold_at,
                        "Item sold_at should not change on second rollback attempt")
        self.assertEqual(self.item.sold_by, after_first_rollback_item_sold_by,
