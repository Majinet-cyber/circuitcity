...
class Sale(models.Model):
    ...
    is_rolled_back = models.BooleanField(
        default=False, db_index=True, help_text="Whether this sale has been rolled back/reversed"
    )
...
class SaleRollback(models.Model):
    ...
    sale = models.ForeignKey(
        Sale, on_delete=models.PROTECT, related_name="rollbacks", help_text="The sale that was rolled back"
    )
    reason = models.CharField(max_length=20, choices=RollbackReason.choices, help_text="Reason for rollback")
    refunded = models.BooleanField(default=False, help_text="Whether a refund was issued to customer")
    ...
    return_to_stock = models.BooleanField(default=False, help_text="Whether item was returned to stock")
    notes = models.TextField(blank=True, help_text="Additional notes about the rollback")
    created_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="created_rollbacks", help_text="User who performed the rollback"
    )
    created_at = models.DateTimeField(default=timezone.now, editable=False, help_text="When the rollback was performed")
...
class SaleCommission(models.Model):
    ...
    is_reversed = models.BooleanField(
        default=False, db_index=True, help_text="Whether this commission was reversed due to sale rollback"
    )
    reversed_at = models.DateTimeField(null=True, blank=True, help_text="When this commission was reversed")
...
class LiquorSaleAttribution(models.Model):
    ...
    liquor_sale_id = models.PositiveIntegerField(
        db_index=True, help_text="ID of the LiquorSale this attribution is for"
    )

    # Business context
    business = models.ForeignKey(
        "tenants.Business", on_delete=models.CASCADE, related_name="liquor_sale_attributions", db_index=True
    )
