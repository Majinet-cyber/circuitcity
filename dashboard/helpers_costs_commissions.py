...
def get_costs_and_commissions_panel(
    business, start_date: date, end_date: date, revenue: Decimal = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            wallet_txns = WalletTxn.objects.filter(
                reason="COMMISSION", created_at__gte=start_date, created_at__lte=end_date
            )
            ...
            try:
                ...
                agent_ids = Membership.objects.filter(business=business, role="AGENT").values_list("user_id", flat=True)
    ...
    result["net_profit_this_period"] = (
        result["gross_profit_this_period"] - result["commissions_this_period"] - result["costs_this_period"]
    )

    # ===== 6. Calculate Profit Margin =====
    if result["revenue_this_period"] > 0:
        result["profit_margin"] = ((result["net_profit_this_period"] / result["revenue_this_period"]) * 100).quantize(
            Decimal("0.01")
        )
    else:
