...
def _get_sales_queryset(business, vertical: Optional[str] = None):
    ...
    try:
        ...
        return Sale.objects.filter(Q(item__business=business) | Q(location__business=business)).select_related("item")
    except Exception:
        return None
...
def get_payment_mix_for_dashboard(business, period_days: int = 30, user=None) -> List[Dict[str, Any]]:
    ...
    return get_payment_mix(business=business, start_date=start_date, end_date=end_date, user=user)
