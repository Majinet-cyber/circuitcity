...
def _predicted_stockouts(business_id, horizon_days=7):
    ...
    qs = Sale.objects.filter(business_id=business_id, ts__gte=cutoff).values("sku_id").annotate(sold=Sum("qty"))
...
def gamified_messages(business_id):
    ...
    if best_day:
        msgs.append(
            {"title": "ðŸ”¥ Best Day Ever", "text": f"You smashed {best_day['total']:,} MK on {best_day['ts__date']}!"}
        )
    streak = (
        Sale.objects.filter(business_id=business_id, ts__date__gte=today - dt.timedelta(days=6))
        .values("ts__date")
        .distinct()
        .count()
    )
