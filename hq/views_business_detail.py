...
def _get_overview_data(business: Business, subscription: Subscription | None) -> dict:
    ...
    paid_total = invoices.filter(status="PAID").aggregate(total=Sum("total"))["total"] or Decimal("0")
    open_total = invoices.filter(status__in=["PENDING", "OPEN"]).aggregate(total=Sum("total"))["total"] or Decimal("0")

    # Sales summary (last 30 days)
    thirty_days_ago = timezone.now() - timedelta(days=30)
    if Sale:
        sales_30d = Sale.objects.filter(location__business=business, created_at__gte=thirty_days_ago).aggregate(
    ...
    # Agents
    agents_count = Membership.objects.filter(business=business, role__in=["AGENT", "MANAGER"]).count()

    # Stock summary (if applicable)
    if InventoryItem:
        stock_count = InventoryItem.objects.filter(business=business, archived_at__isnull=True).count()
        stock_in_7d = InventoryItem.objects.filter(
            business=business, archived_at__isnull=True, created_at__gte=timezone.now() - timedelta(days=7)
        ).count()
    else:
        stock_count = 0
        stock_in_7d = 0
...
def _get_data_inventory_data(business: Business) -> dict:
    ...
    stock_items = InventoryItem.objects.filter(business=business, archived_at__isnull=True).order_by("-created_at")[:20]

    archived_count = InventoryItem.objects.filter(business=business, archived_at__isnull=False).count()
...
def _get_sales_wallet_data(business: Business) -> dict:
    ...
    thirty_days_ago = timezone.now() - timedelta(days=30)
    if Sale:
        recent_sales = Sale.objects.filter(location__business=business, created_at__gte=thirty_days_ago).order_by(
            "-created_at"
        )[:20]
    else:
        recent_sales = []
    ...
    # Wallet transactions
    wallet_txns = WalletTransaction.objects.filter(business=business).order_by("-created_at")[:20]

    # Summary
    wallet_summary = WalletTransaction.objects.filter(business=business).aggregate(
        total_income=Sum("amount", filter=Q(amount__gt=0)), total_expense=Sum("amount", filter=Q(amount__lt=0))
    )
...
def _get_tickets_data(business: Business) -> dict:
    ...
    open_tickets_count = SupportTicket.objects.filter(business=business, status__in=["open", "in_progress"]).count()
...
def _get_audit_data(business: Business) -> dict:
    ...
    audit_logs = SupportActionLog.objects.filter(business=business).select_related("actor").order_by("-created_at")[:50]
...
def add_business_note(request: HttpRequest, business_id: int) -> HttpResponse:
    ...
    BusinessNote.objects.create(business=business, author=request.user, title=title, content=content, is_pinned=True)
...
def _get_chart_data(business: Business) -> dict:
    ...
    if Sale:
        sales_by_day = (
            Sale.objects.filter(location__business=business, created_at__gte=thirty_days_ago)
    ...
    sale_amounts = []
    if Sale:
        recent_sales = Sale.objects.filter(location__business=business, created_at__gte=thirty_days_ago).values_list(
            "price", flat=True
        )
        sale_amounts = [float(amt) for amt in recent_sales if amt]

    # Create bins for histogram
    sale_distribution = {"0-1000": 0, "1000-5000": 0, "5000-10000": 0, "10000-50000": 0, "50000+": 0}
    ...
    if SupportTicket:
        tickets_opened = SupportTicket.objects.filter(business=business, created_at__gte=thirty_days_ago).count()

        tickets_closed = SupportTicket.objects.filter(
            business=business, status__in=["resolved", "closed"], resolved_at__gte=thirty_days_ago
        ).count()

    return {
        "sales_trend": {"labels": sales_trend_labels or ["No data"], "data": sales_trend_data or [0]},
        "transactions_by_type": transactions_by_type,
        "sale_distribution": sale_distribution,
        "tickets": {"opened": tickets_opened, "closed": tickets_closed},
    }
