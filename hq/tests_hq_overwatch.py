...
class HQPermissionsTestCase(TestCase):
    ...
    def test_hq_admin_can_access_command_center(self):
        ...
        response = self.client.get(
            reverse("hq:business_command_center", kwargs={"business_id": self.business.id})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_hq_admin_can_access_account_support(self):
        ...
        self.client.force_login(self.hq_admin)
        response = self.client.get(
            reverse("hq:account_support", kwargs={"business_id": self.business.id})
        )
        self.assertEqual(response.status_code, 200)
...
class RedirectLoopRegressionTestCase(TestCase):
    ...
    def test_hq_businesses_returns_200_not_redirect(self):
        ...
        self.assertEqual(
            ...
            f"Location header: {response.get('Location', 'None')}"
        )
    ...
    def test_superuser_bypasses_tenant_middleware(self):
        ...
        self.assertEqual(
            response.status_code, 
            200,
            f"/hq/businesses/ returned {response.status_code}"
        )
        ...
        for redirect_url, _ in response.redirect_chain:
            self.assertNotIn(
                "/tenants/choose",
                redirect_url,
                "/hq/businesses/ redirected to /tenants/choose/ which causes loops"
            )
    ...
    def test_prevent_hq_middleware_loop_guard(self):
        ...
        self.assertEqual(
            response.status_code,
            200,
            f"/hq/businesses/ returned {response.status_code} instead of 200"
        )
...
class HQTemplateDataContractTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.business_with_sub = Business.objects.create(
            name="Business With Subscription",
            slug="biz-with-sub",
            created_by=self.hq_admin,
            status="ACTIVE"
        )
    ...
    def test_business_directory_renders_without_template_var_errors(self):
        ...
        self.assertEqual(
            response.status_code, 
            200,
            f"Business directory returned {response.status_code}, expected 200"
        )
        ...
        for biz in response.context["businesses"]:
            ...
            for key in required_keys:
                self.assertIn(
                    key, 
                    biz.sub_state, 
                    f"Business {biz.name} sub_state missing key: {key}"
                )
    ...
    def test_business_detail_handles_missing_subscription(self):
        ...
        response = self.client.get(
            reverse("hq:business_detail", kwargs={"pk": self.business_no_sub.id})
        )
        ...
        self.assertEqual(
            response.status_code, 
            200,
            f"Business detail (no sub) returned {response.status_code}, expected 200"
        )
        ...
        response = self.client.get(
            reverse("hq:business_detail", kwargs={"pk": self.business_with_sub.id})
        )
    ...
    def test_hq_home_dashboard_renders_sqlite_safe(self):
        ...
        self.assertEqual(
            response.status_code, 
            200,
            f"/hq/home/ returned {response.status_code}, expected 200"
        )
    ...
    def test_hq_agents_renders_successfully(self):
        ...
        self.assertEqual(
            response.status_code, 
            200,
            f"/hq/agents/ returned {response.status_code}, expected 200"
        )
...
class SQLiteDateTimeSafetyTestCase(TestCase):
    ...
    def test_monthly_drill_down_api_sqlite_safe(self):
        ...
        import datetime
        response = self.client.get(
            reverse("hq:monthly_drill_down_api"),
            {"year": datetime.date.today().year, "month": 1}
        )
