...
def account_support_home(request: HttpRequest, business_id: int) -> HttpResponse:
    ...
    memberships = Membership.objects.filter(business=business).select_related("user").order_by("-role", "user__email")
...
def reset_password_for_user(request: HttpRequest, business_id: int, user_id: int) -> HttpResponse:
    ...
    try:
        ...
        code = PasswordResetCode.objects.create(email=user.email, expires_at=timezone.now() + timedelta(hours=24))

        # Generate reset link
        from django.urls import reverse

        reset_url = request.build_absolute_uri(reverse("accounts:password_reset_confirm", kwargs={"code": code.code}))
        ...
        messages.success(request, f"Password reset link generated for {user.email}: {reset_url}")
    except Exception as e:
...
def resend_otp(request: HttpRequest, business_id: int, user_id: int) -> HttpResponse:
    ...
    try:
        ...
        otp = EmailOTP.objects.create(
            email=user.email, purpose="login", expires_at=timezone.now() + timedelta(minutes=10)
        )
