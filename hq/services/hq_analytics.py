...
def _build_sales_queryset(
    ...
    if vertical:
        ...
        qs = qs.filter(
            Q(item__business__business_kind__iexact=vertical) |
            Q(item__business__vertical__iexact=vertical)
        )
...
def _get_kpis(
    ...
    cost_data = sales_qs.annotate(cost=cost_annotation).aggregate(
        total_cost=Coalesce(Sum('cost'), zero_dec)
    )
    ...
    if vertical:
        active_businesses = active_businesses.filter(
            Q(business_kind__iexact=vertical) | Q(vertical__iexact=vertical)
        )
    ...
    if not vertical or vertical.lower() != 'gym':
        ...
        if vertical:
            ...
            inv_qs = inv_qs.filter(
                Q(business__business_kind__iexact=vertical) |
                Q(business__vertical__iexact=vertical)
            )
...
def _get_chart_series(
    ...
    if connection.vendor == 'sqlite':
        ...
        daily_revenue = [
            {'day': day, 'revenue': data['revenue'], 'count': data['count']}
            for day, data in sorted(daily_data.items())
        ]
    else:
        # PostgreSQL/MySQL: use DB-level aggregation
        daily_revenue = sales_qs.annotate(
    ...
    if connection.vendor == 'sqlite':
        ...
        sales_for_profit = sales_qs.select_related('item').values('sold_at', 'price', 'item__cost', 'item__purchase_price')
        ...
        daily_profit = sales_qs.annotate(
            day=TruncDate('sold_at'),
            profit=profit_annotation
        ).values('day').annotate(
            total_profit=Coalesce(Sum('profit'), Value(0, output_field=DecimalField(max_digits=12, decimal_places=2)))
        ).order_by('day')
...
def _get_breakdowns(
    ...
    if not business_id:  # Only show if not filtering by business
        ...
        vertical_agg = sales_qs.values(
            'item__business__business_kind',
            'item__business__vertical'
        ).annotate(
...
def _get_top_lists(
    ...
    top_businesses = []
    if not business_id:  # Only show if not filtering by business
        business_agg = sales_qs.values(
            'item__business_id',
            'item__business__name'
        ).annotate(
            revenue=Coalesce(Sum('price'), zero_dec),
            count=Count('id')
        ).order_by('-revenue')[:10]
