...
class MembershipExtendTest(TestCase):
    ...
    def setUp(self):
        ...
        self.plan = SubscriptionPlan.objects.create(
            code="starter", name="Starter Plan", amount=Decimal("20000.00"), interval="month"
        )
        self.subscription = BusinessSubscription.objects.create(
            business=self.business, plan=self.plan, status="trial", trial_end=timezone.now() + timedelta(days=7)
        )
        self.staff_user = User.objects.create_user(
            username="hq_staff", password="testpass123", is_staff=True, is_superuser=True
        )
        self.client = Client()
    ...
    def test_extend_subscription_by_days(self):
        ...
        response = self.client.get(reverse("hq:sub_extend", args=[self.subscription.pk]), {"days": days_to_add})
    ...
    def test_extend_subscription_to_specific_date(self):
        ...
        response = self.client.get(
            reverse("hq:sub_extend", args=[self.subscription.pk]), {"trial_end": target_date.isoformat()}
        )
    ...
    def test_extend_subscription_creates_audit_log(self):
        ...
        response = self.client.get(reverse("hq:sub_extend", args=[self.subscription.pk]), {"days": 15})
    ...
    def test_extend_subscription_negative_days(self):
        ...
        response = self.client.get(reverse("hq:sub_extend", args=[self.subscription.pk]), {"days": days_to_subtract})
...
class MembershipRevokeTest(TestCase):
    ...
    def setUp(self):
        ...
        self.plan = SubscriptionPlan.objects.create(
            code="pro", name="Pro Plan", amount=Decimal("35000.00"), interval="month"
        )
        self.subscription = BusinessSubscription.objects.create(
        ...
        self.staff_user = User.objects.create_user(
            username="hq_admin", password="testpass123", is_staff=True, is_superuser=True
        )
        self.client = Client()
        self.client.force_login(self.staff_user)
...
class MembershipActivateTest(TestCase):
    ...
    def setUp(self):
        ...
        self.plan = SubscriptionPlan.objects.create(
            code="promax", name="Pro Max Plan", amount=Decimal("50000.00"), interval="month"
        )
        self.subscription = BusinessSubscription.objects.create(
            business=self.business, plan=self.plan, status="canceled"
        )
        self.staff_user = User.objects.create_user(
            username="hq_superuser", password="testpass123", is_staff=True, is_superuser=True
        )
        self.client = Client()
        self.client.force_login(self.staff_user)
...
class MembershipUITest(TestCase):
    ...
    def setUp(self):
        ...
        self.subscription = BusinessSubscription.objects.create(
            business=self.business, plan=self.plan, status="trial", trial_end=timezone.now() + timedelta(days=10)
        )
        self.staff_user = User.objects.create_user(
            username="ui_staff", password="testpass123", is_staff=True, is_superuser=True
        )
        self.client = Client()
        self.client.force_login(self.staff_user)
...
class MembershipPermissionsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.plan = SubscriptionPlan.objects.create(code="starter", name="Starter Plan", amount=Decimal("20000.00"))
        self.subscription = BusinessSubscription.objects.create(business=self.business, plan=self.plan, status="trial")
    ...
    def test_extend_requires_staff_permission(self):
        ...
        response = self.client.get(reverse("hq:sub_extend", args=[self.subscription.pk]), {"days": 30})
...
class SubscriptionStatusDisplayTest(TestCase):
    ...
    def setUp(self):
        self.staff_user = User.objects.create_user(
            username="badge_staff", password="testpass123", is_staff=True, is_superuser=True
        )
        self.client = Client()
        self.client.force_login(self.staff_user)
    ...
    def test_trial_status_badge(self):
        ...
        subscription = BusinessSubscription.objects.create(
            business=business, plan=plan, status="trial", trial_end=timezone.now() + timedelta(days=7)
        )
    ...
    def test_active_status_badge(self):
        ...
        subscription = BusinessSubscription.objects.create(
            business=business, plan=plan, status="active", current_period_end=timezone.now() + timedelta(days=30)
        )
    ...
    def test_canceled_status_badge(self):
        ...
        plan = SubscriptionPlan.objects.create(code="starter", name="Starter", amount=Decimal("20000"))
        subscription = BusinessSubscription.objects.create(business=business, plan=plan, status="canceled")
