...
class IntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.hq_admin = User.objects.create_superuser(
            username="hqadmin", email="hqadmin@test.com", password="password123"
        )
        ...
        # Create memberships
        Membership.objects.create(user=self.agent1, business=self.business, role="AGENT")
        Membership.objects.create(user=self.agent2, business=self.business, role="AGENT")

        # Create sales data
        today = timezone.now()
        for i in range(25):
            Sale.objects.create(agent=self.agent1, price=Decimal("150.00"), sold_at=today)
        for i in range(15):
            Sale.objects.create(agent=self.agent2, price=Decimal("120.00"), sold_at=today)
    ...
    def test_milestone_awarding(self):
        ...
        milestones = check_and_award_milestones(
            user=self.agent1, business=self.business, sales_count=25, year=year, month=month
        )
        ...
        # Check database
        db_milestones = AgentMilestone.objects.filter(user=self.agent1, business=self.business)
    ...
    def test_multi_tenant_isolation(self):
        ...
        agent3 = User.objects.create_user(username="agent3", email="agent3@test.com", password="password123")
        Membership.objects.create(user=agent3, business=business2, role="AGENT")

        # Create sales for business2
        today = timezone.now()
        for i in range(10):
            Sale.objects.create(agent=agent3, price=Decimal("100.00"), sold_at=today)
