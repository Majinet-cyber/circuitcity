...
class HQContractsViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.business_with_contract = Business.objects.create(
            name="Business With Contract",
            slug="biz-with-contract",
            status="ACTIVE"
        )
        
        self.business_without_contract = Business.objects.create(
            name="Business Without Contract",
            slug="biz-no-contract",
            status="ACTIVE"
        )
        ...
        self.contract = MerchantContract.objects.create(
            business=self.business_with_contract,
            title="Test Contract",
            notes="Test notes",
    ...
    def test_contracts_list_shows_correct_status(self):
        ...
        biz_with_contract = next(
            (item for item in businesses_with_status 
             if item['business'].id == self.business_with_contract.id),
            None
        )
        biz_without_contract = next(
            (item for item in businesses_with_status 
             if item['business'].id == self.business_without_contract.id),
            None
        )
...
class HQContractUploadTests(TestCase):
    ...
    def test_contract_upload_creates_new_contract(self):
        ...
        self.assertTrue(
            MerchantContract.objects.filter(business=self.business).exists()
        )
