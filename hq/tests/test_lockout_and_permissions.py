...
class SubscriptionLockoutTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.plan = SubscriptionPlan.objects.create(name="Test Plan", amount=20000, is_active=True)
    ...
    def test_lockout_blocks_inventory_pages(self):
        ...
        for page in pages_to_test:
            ...
            # Should be blocked
            self.assertIn(
                response.status_code, [403, 302], f"Page {page} should be blocked but got {response.status_code}"
            )
...
class RoleBasedUIPermissionsTestCase(TestCase):
    ...
    def test_manager_sees_subscription_actions(self):
        ...
        if response.status_code == 200:
            ...
            self.assertTrue(
                "subscription-actions" in content or "Choose plan" in content or "Checkout" in content,
        ...
        for endpoint in json_endpoints:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    self.assertFalse(
                        data.get("ok", True), f"JSON endpoint {endpoint} should return error during lockout"
                    )
                except:
                    pass
            else:
