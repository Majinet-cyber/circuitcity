...
class TestPharmacyFastSellNoRecursion(TestCase):
    ...
    def test_pharmacy_fast_sell_page_loads(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200,
                           "Pharmacy fast sell should return 200")
            
            # Check that template rendered (not empty)
            self.assertTrue(len(response.content) > 0,
                          "Fast sell page should have content")
            
            # Check for key elements (scanner, payment mix, etc.)
            content = response.content.decode("utf-8")
            self.assertIn("fast-sell", content.lower(),
                         "Page should contain fast sell elements")
    ...
    def test_pharmacy_dashboard_loads(self):
        ...
        try:
            ...
            # Should return 200
            self.assertEqual(response.status_code, 200,
                           "Pharmacy dashboard should return 200")
    ...
    def test_pharmacy_hub_loads(self):
        ...
        try:
            ...
            # Should return 200
            self.assertEqual(response.status_code, 200,
                           "Pharmacy hub should return 200")
    ...
    def test_pharmacy_fast_sell_api_endpoints(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200,
                           "Fast sell lookup API should return 200")
        ...
        try:
            response = self.client.get(
                reverse("verticals:pharmacy_fast_sell_kpis_api"),
                {"range": "today"}
            )
            
            # Should return 200
            self.assertEqual(response.status_code, 200,
                           "Fast sell KPIs API should return 200")
    ...
    def test_redirect_chain_no_loop(self):
        ...
        try:
            response = self.client.get(
                reverse("verticals:pharmacy_dashboard"),
                follow=True
            )
            self.assertEqual(response.status_code, 200,
                           "Pharmacy dashboard redirect chain should end in 200")
            
            # Check redirect chain length (should be short, not infinite)
            if hasattr(response, "redirect_chain"):
                self.assertLess(len(response.redirect_chain), 5,
                              "Should not have long redirect chain")
        except RecursionError as e:
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200,
                           "/inventory/dashboard/ should resolve")
        except RecursionError as e:
    ...
    def test_pharmacy_fast_sell_production_mode(self):
        ...
        try:
            ...
            self.assertEqual(response.status_code, 200,
                           "Fast sell should work in production mode")
...
class TestPharmacyTemplateIncludes(TestCase):
    ...
    def test_payment_mix_bar_include_works(self):
        ...
        try:
            ...
            content = response.content.decode("utf-8")
            self.assertIn("payment", content.lower(),
                         "Payment mix should be included")
    ...
    def test_base_html_extends_works(self):
        ...
        try:
            ...
            self.assertIn("<!doctype html>", content.lower(),
                         "Should have HTML doctype from base.html")
