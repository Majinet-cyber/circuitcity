    ...
    try:
        ...
        if hasattr(instance, "item") and instance.item:
            ...
            if hasattr(item, "product") and item.product:
                ...
                product_name = f"{getattr(product, 'brand', '')} {getattr(product, 'model', '')}".strip() or str(
                    product
                )
        ...
        if business:
            ...
            managers = Membership.objects.filter(
                business=business, role__in=["MANAGER", "ADMIN"], status="ACTIVE"
...
def notify_new_agent(sender, instance, created, **kwargs):
    ...
    try:
        ...
        managers = (
            Membership.objects.filter(business=instance.business, role="MANAGER", status="ACTIVE")
        ...
        for manager_id in managers:
            Notification.objects.create(
                ...
                level="info",
                meta={"type": "new_agent", "agent_id": instance.user.id, "agent_username": instance.user.username},
            )
    except Exception as e:
        import logging
...
def notify_low_stock(sender, instance, created, **kwargs):
    ...
    try:
        ...
        if stock <= threshold and stock > 0:
            ...
            # Notify managers
            managers = Membership.objects.filter(business=business, role="MANAGER", status="ACTIVE").values_list(
                "user_id", flat=True
            )
