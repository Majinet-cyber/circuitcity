...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--limit", type=int, default=100, help="Maximum number of failed events to retry (default: 100)"
        )
        parser.add_argument(
            "--dry-run", action="store_true", help="Show what would be retried without actually retrying"
        )
    ...
    def handle(self, *args, **options):
        ...
        if count == 0:
            self.stdout.write(self.style.SUCCESS("No failed email events found."))
            return

        self.stdout.write(f"Found {count} failed email event(s) to retry.")

        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN: Would retry the following events:"))
        ...
        for event in failed_events:
            try:
                ...
                retried += 1
                self.stdout.write(self.style.SUCCESS(f"✓ Retried: {event.event_type} -> {event.recipient_email}"))
            except Exception as e:
                failed += 1
                self.stdout.write(self.style.ERROR(f"✗ Failed to retry event {event.id}: {e}"))
