    ...
    def handle(self, *args, **options):
        ...
        if today.day != 27:
            self.stdout.write(self.style.WARNING(
                f"Today is the {today.day}th, not the 27th. No alerts will be created."
            ))
            if not dry_run:
                return
        ...
        try:
            ...
            if count > 0:
                self.stdout.write(self.style.SUCCESS(
                    f"✓ Successfully created {count} payslip alert(s)"
                ))
            else:
                self.stdout.write(self.style.WARNING(
                    "No new alerts created (either not the 27th or all users already notified)"
                ))
