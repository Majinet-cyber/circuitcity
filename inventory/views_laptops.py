...
def laptop_stock_in(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        existing = LaptopSerial.objects.filter(business=business, serial=serial, is_active=True).first()
        ...
        messages.success(request, f"✅ Laptop '{laptop_product.display_name}' (Serial: {serial}) stocked successfully!")
        return redirect("inventory:laptop_stock_in")

    # GET: Show form
    # Get existing laptop products for quick selection
    existing_products = LaptopProduct.objects.filter(business=business, is_active=True).order_by("brand", "model_name")[
        :50
    ]
...
def laptop_sell(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        try:
            laptop_serial = LaptopSerial.objects.get(pk=serial_id, business=business, status="IN_STOCK", is_active=True)
    ...
    available_laptops = (
        LaptopSerial.objects.filter(business=business, status="IN_STOCK", is_active=True)
        .select_related("product", "location")
        .order_by("product__brand", "product__model_name")
    )
...
def laptop_products_list(request: HttpRequest) -> HttpResponse:
    ...
    products = LaptopProduct.objects.filter(business=business, is_active=True).order_by("brand", "model_name")

    # Get stock counts per product
    for product in products:
        product.stock_count = LaptopSerial.objects.filter(product=product, status="IN_STOCK", is_active=True).count()
