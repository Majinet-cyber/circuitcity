...
def get_stock_alerts(business, location=None, threshold: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        stock_qs = InventoryItem.objects.filter(business=business, status="IN_STOCK", is_active=True).select_related(
            "product"
        )
        ...
        product_counts = (
            stock_qs.values(
                "product_id", "product__brand", "product__model", "product__variant", "product__low_stock_threshold"
            )
...
def get_cfo_alerts(business, location=None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        sold_items = InventoryItem.objects.filter(business=business, status="SOLD", sold_at__gte=thirty_days_ago)

        if location:
            sold_items = sold_items.filter(current_location=location)

        # Group by product and calculate daily run-rate
        sales_by_product = sold_items.values("product_id", "product__brand", "product__model").annotate(
            sold_count=Count("id")
        )

        # Get current stock
        stock_qs = InventoryItem.objects.filter(business=business, status="IN_STOCK", is_active=True)

        if location:
            stock_qs = stock_qs.filter(current_location=location)

        stock_by_product = {
            item["product_id"]: item["on_hand"] for item in stock_qs.values("product_id").annotate(on_hand=Count("id"))
        }
...
def get_ai_insights(business, location=None) -> Dict[str, Any]:
    ...
    try:
        ...
        sold_qs = InventoryItem.objects.filter(business=business, status="SOLD", sold_at__gte=thirty_days_ago)
        ...
        # Products with stock
        products_with_stock = set(stock_qs.values_list("product_id", flat=True).distinct())

        # Products that sold recently
        products_sold_recently = set(sold_qs.values_list("product_id", flat=True).distinct())
...
def get_revenue_profit_summary(business, location=None, period="this_month") -> Dict[str, Any]:
    ...
    try:
        ...
        sold_qs = InventoryItem.objects.filter(
            business=business, status="SOLD", sold_at__gte=start_dt, sold_at__lt=end_dt
        )
