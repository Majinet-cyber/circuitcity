    ...
    if business:
        try:
            membership = Membership.objects.filter(
                user=user, business=business, role="MANAGER", status="ACTIVE"
            ).first()
            if membership:
                return True
        except Exception:
    ...
    with transaction.atomic():
        if owner_id:
            ...
            try:
                ...
                membership = Membership.objects.filter(
                    user=agent, business=business, role="AGENT", status="ACTIVE"
                ).first()
                ...
                messages.success(request, f"Stock assigned to {agent.get_full_name() or agent.username}")
...
def bulk_assign_stock(request: HttpRequest) -> JsonResponse:
    ...
    with transaction.atomic():
        items = InventoryItem.objects.filter(pk__in=stock_ids, business=business, status="IN_STOCK")
        ...
        if owner_id:
            try:
                ...
                if not membership:
                    return JsonResponse(
                        {"ok": False, "error": f"User is not an active agent for this business"}, status=400
                    )

                updated = items.update(assigned_agent=agent, assigned_role="AGENT")
            ...
            updated = items.update(assigned_agent=None, assigned_role="MANAGER")

            return JsonResponse({"ok": True, "updated": updated, "message": f"Reclaimed {updated} items to Manager"})
...
def get_business_agents(request: HttpRequest) -> JsonResponse:
    ...
    memberships = Membership.objects.filter(business=business, role="AGENT", status="ACTIVE").select_related(
        "user", "location"
    )
