...
def backfill_internal_sku(apps, schema_editor):
    ...
    products_needing_sku = MerchProduct.objects.filter(Q(internal_sku="") | Q(internal_sku__isnull=True))
    ...
    for product in products_needing_sku:
        product.internal_sku = generate_sku(business_id=product.business_id, name=product.name)
