...
def find_sellable_by_barcode(barcode: str, business, vertical: str = None):
    ...
    try:
        ...
        qs = InventoryItem.objects.filter(business=business, status="IN_STOCK").select_related("product")
        ...
        if vertical == "pharmacy":
            try:
                ...
                batches = (
                    PharmacyBatch.objects.filter(business=business, quantity__gt=0)
                    .filter(models.Q(batch_barcode=barcode) | models.Q(product__barcode=barcode))
                ...
                if batches.exists():
                    ...
                    return qs.filter(product=batch.product, batch_id=batch.id)
            except Exception:
                pass
...
def register_barcode(business, raw_code: str, product=None, batch=None, created_by=None):
    """
    ...
    try:
        ...
        existing = (
            BarcodeRegistry.objects.for_business(business).filter(normalized_code=normalized, is_active=True).first()
        )
...
def lookup_barcode(business, raw_code: str, location=None):
    ...
    try:
        ...
        # Use for_business to explicitly scope the query (bypasses TenantManager's thread-local filtering)
        reg = (
            BarcodeRegistry.objects.for_business(business)
            .filter(normalized_code=normalized, is_active=True)
            .select_related("product", "batch")
            .first()
        )

        if reg:
            return {"found": True, "product": reg.product, "batch": reg.batch, "registry_entry": reg}
        ...
        products = (
            MerchProduct.objects.filter(business=business, is_active=True)
        ...
        if mp:
            ...
            return {"found": True, "product": mp, "batch": None, "registry_entry": reg_entry}
