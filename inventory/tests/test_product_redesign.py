...
class ProductListingRegressionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.business = Business.objects.create(name="Test Business", kind=BusinessKind.LIQUOR, owner=self.user)
        self.location = Location.objects.create(business=self.business, name="Main Location")
...
class PharmacyGameficationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="pharmacist", password="testpass123", email="pharmacist@example.com"
        )
        self.business = Business.objects.create(name="Test Pharmacy", kind=BusinessKind.PHARMACY, owner=self.user)
        profile, _ = UserProfile.objects.get_or_create(user=self.user)
        profile.active_business = self.business
        profile.save()
...
class MobileResponsivenessTests(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username="mobile_user", password="testpass123")
        self.business = Business.objects.create(name="Mobile Business", kind=BusinessKind.LIQUOR, owner=self.user)
        profile, _ = UserProfile.objects.get_or_create(user=self.user)
        profile.active_business = self.business
        profile.save()
...
class TemplateNoRegressionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.business = Business.objects.create(name="Edge Case Business", kind=BusinessKind.LIQUOR, owner=self.user)
        profile, _ = UserProfile.objects.get_or_create(user=self.user)
        profile.active_business = self.business
        profile.save()
    ...
    def test_product_with_very_long_name_truncates(self):
        ...
        product = MerchProduct.objects.create(
            business=self.business, kind=BusinessKind.LIQUOR, name=long_name, category="beer"
        )
