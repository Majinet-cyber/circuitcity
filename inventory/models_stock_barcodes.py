...
class InventoryBarcode(models.Model):
    ...
    created_by = models.ForeignKey(
        "auth.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="created_stock_barcodes"
    )

    # Archive support
    is_archived = models.BooleanField(
        default=False, db_index=True, help_text="Set to True to archive this barcode without deleting"
    )
    archived_at = models.DateTimeField(null=True, blank=True, db_index=True, help_text="When this barcode was archived")
    archived_by = models.ForeignKey(
        "auth.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="archived_stock_barcodes"
    )
...
class ArchiveBatch(models.Model):
    ...
    business = models.ForeignKey(Business, on_delete=models.CASCADE, related_name="archive_batches", db_index=True)
    ...
    created_by = models.ForeignKey(
        "auth.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="created_archive_batches"
    )
    ...
    counts_snapshot = models.JSONField(
        default=dict, help_text="JSON snapshot of affected records: products, stock_items, barcodes, etc."
    )
