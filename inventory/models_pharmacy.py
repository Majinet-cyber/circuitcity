...
class PharmacyProductInfo(models.Model):
    ...
    strength = models.CharField(max_length=50, blank=True, default="", help_text="e.g. 500mg, 10ml, 250mg/5ml")
    form = models.CharField(
        max_length=20, choices=PharmacyProductForm.choices, default=PharmacyProductForm.TABLET, help_text="Dosage form"
    )
    category = models.CharField(
        max_length=30, choices=PharmacyCategory.choices, blank=True, default="", help_text="Drug category"
    )

    # Prescription requirement
    requires_prescription = models.BooleanField(default=False, help_text="True if this drug requires a prescription")

    # Generic vs Brand
    is_generic = models.BooleanField(default=False, help_text="True if this is a generic (non-branded) drug")
    active_ingredient = models.CharField(
        max_length=255, blank=True, default="", help_text="Active pharmaceutical ingredient (API)"
    )

    # Storage requirements
    storage_notes = models.TextField(
        blank=True, default="", help_text="Special storage requirements (e.g. refrigerate, keep dry)"
    )
...
class PharmacyBatch(models.Model):
    ...
    business = models.ForeignKey(Business, on_delete=models.CASCADE, related_name="pharmacy_batches")

    # Batch identification
    batch_number = models.CharField(
        max_length=100, blank=True, default="", help_text="Manufacturer batch/lot number (optional)"
    )
    barcode = models.CharField(
        max_length=100, blank=True, default="", db_index=True, help_text="Batch-specific barcode for Fast Sell lookup"
    )
    expiry_date = models.DateField(
        db_index=True, blank=True, null=True, help_text="Expiry date (day/month/year, optional for cosmetics)"
    )

    # Stock levels
    quantity = models.PositiveIntegerField(default=0, help_text="Current quantity in stock")
    reorder_level = models.PositiveIntegerField(default=10, help_text="Alert when stock drops to or below this level")

    # Pricing
    cost_price = models.DecimalField(
        max_digits=10, decimal_places=2, validators=[MinValueValidator(0)], help_text="Cost per unit"
    )
    selling_price = models.DecimalField(
        max_digits=10, decimal_places=2, validators=[MinValueValidator(0)], help_text="Selling price per unit"
    )

    # Supplier info
    supplier = models.CharField(max_length=200, blank=True, default="", help_text="Supplier name")

    # Timestamps
    received_date = models.DateField(default=timezone.now, help_text="Date batch was received")
...
class PharmacySale(models.Model):
    ...
    batch = models.ForeignKey(
        PharmacyBatch, on_delete=models.PROTECT, related_name="sales", help_text="Batch sold from"
    )

    # Sale details
    quantity = models.PositiveIntegerField(validators=[MinValueValidator(1)], help_text="Quantity sold")
    ...
    # Customer (optional)
    customer_name = models.CharField(max_length=200, blank=True, default="", help_text="Customer name (optional)")
    customer_phone = models.CharField(max_length=20, blank=True, default="", help_text="Customer phone (optional)")

    # Prescription (if required)
    prescription_number = models.CharField(
        max_length=100, blank=True, default="", help_text="Prescription reference (if applicable)"
    )

    # Who made the sale
    sold_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name="pharmacy_sales")
    ...
    is_deleted = models.BooleanField(
        default=False, db_index=True, help_text="Soft delete flag - deleted sales excluded from reports"
    )
    is_reversed = models.BooleanField(default=False, db_index=True, help_text="True if this sale was reversed/undone")
    ...
    deleted_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL, related_name="pharmacy_sales_deleted"
    )
