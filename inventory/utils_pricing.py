...
def validate_phone_selling_price(
    selling_price: Decimal, is_blocking: bool = True
    ...
    if selling_price < min_price:
        ...
        if is_blocking:
            ...
            msg = f"That looks too low (MK {selling_price:,.0f}). " f"Did you mean {selling_price * 1000:,.0f}?"
            return False, msg, suggested_price if suggested_valid else None
...
def validate_selling_price(
    ...
    if cost_price and cost_price > 0:
        ...
        if selling_price < cost_price:
            ...
            result["warnings"].append(f"⚠️ This is below order value ({format_currency(cost_price)}).")
        ...
        # 3. Very low margin warning (< 5%)
            ...
            result["warnings"].append(f"⚠️ Low profit margin ({profit_margin_pct:.1f}%). Consider pricing higher.")
            result["severity"] = "warning"
...
def get_pricing_intelligence_json(
    selling_price: Decimal, cost_price: Optional[Decimal] = None, suggested_price: Optional[Decimal] = None
) -> Dict[str, Any]:
    """
