...
def dashboard_v2(request):
    ...
    # Stock value
    stock_value = sum((p.quantity_in_stock or 0) * (p.cost_price or Decimal("0")) for p in products)
    ...
    dead_stock_count = products.filter(quantity_in_stock__gt=0).exclude(id__in=sold_product_ids).count()

    # PHASE 5: Gamification stats
    from inventory.services_gamification import get_agent_stats

    agent_stats = get_agent_stats(business=business, agent=request.user, days=30)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if not created:
                    ...
                    messages.success(request, f"✅ Product '{full_name}' created successfully!")
...
def stock_in_v2(request):
    ...
    if q:
        products = products.filter(Q(name__icontains=q) | Q(barcode__iexact=q) | Q(sku__icontains=q))
...
def sell_submit_v2(request):
    ...
    try:
        ...
        # PHASE 5: Process gamification (XP, streak, badges)
        from inventory.services_gamification import process_sale_gamification
        ...
        response_data = {
            ...
            # Gamification data
            "gamification": {
                "xp_gained": gamification_result["xp"]["xp_gained"],
                "total_xp": gamification_result["xp"]["total_xp"],
                "level": gamification_result["xp"]["level"],
                "level_up": gamification_result["xp"]["level_up"],
                "old_level": gamification_result["xp"]["old_level"],
                "streak_days": gamification_result["streak"].current_streak_days,
                "badges_earned": [
                    {
                        "name": badge.name,
                        "icon": badge.icon,
                    ...
                    for badge in gamification_result["badges_earned"]
                ],
                "show_celebration": gamification_result["show_celebration"],
            },
        }
...
# ==============================================================================
# LEADERBOARD (PHASE 5 - Gamification)
# ==============================================================================
...
def leaderboard_v2(request):
    """
    PHASE 5: Daily leaderboard showing top performers.
    Shows today's rankings + current user's stats.
    """
    business = get_active_business(request)
    today = timezone.now().date()

    # Generate/get today's leaderboard
    from inventory.services_gamification import generate_daily_leaderboard, get_agent_stats

    leaderboard = generate_daily_leaderboard(business=business, date=today)

    # Get current user's stats
    user_stats = get_agent_stats(business=business, agent=request.user, days=30)

    # Get current user's rank today (if in top 10)
    user_rank = next((entry for entry in leaderboard if entry.agent_id == request.user.id), None)
    ...
    return render(request, "verticals/groceries_v2/leaderboard.html", context)
