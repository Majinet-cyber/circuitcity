...
from inventory.verticals.groceries_seed import (
    CATEGORIES, SEED_ITEMS, get_category_by_key, get_items_by_category,
    get_item_by_key, get_variant_by_label, get_seed_key, parse_seed_key
)
    ...
    if has_total_price:
        ...
        revenue_result = today_sales.aggregate(
            total_revenue=Coalesce(
                Sum(F('quantity') * F('unit_price'), output_field=dec_field),
                Value(0),
        ...
        profit_result = today_sales.aggregate(
            ...
            cost_sum=Coalesce(
                Sum(F('quantity') * F('unit_cost'), output_field=dec_field),
                Value(0),
                output_field=dec_field
            )
        )
