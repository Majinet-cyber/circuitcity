...
def accessories_dashboard(request):
    ...
    # ==========================================================================
    stock_qs = AccessoryStock.objects.filter(business=business, product__is_active=True).select_related(
        "product", "location"
    )
    ...
    total_stock_units = stock_qs.aggregate(total=Coalesce(Sum("qty_on_hand"), 0))["total"] or 0
    ...
    # ==========================================================================
    top_accessories = (
        sale_logs.values("product__id", "product__name", "product__category")
        .annotate(
        ...
        top_accessories_list.append(
            {
                ...
                "category": AccessoryCategory(item["product__category"]).label
    ...
    # ==========================================================================
    recent_movements = AccessoryStockLog.objects.filter(business=business).select_related(
        "product", "location", "by_user"
    )
...
def accessories_stock_in(request):
    ...
    categories = [
        {"value": cat[0], "label": cat[1], "icon": _get_category_icon(cat[0])} for cat in AccessoryCategory.choices
    ]

    # Get existing products grouped by category for quick access
    products_by_category = {}
    for cat_value, cat_label in AccessoryCategory.choices:
        products = AccessoryProduct.objects.filter(business=business, category=cat_value, is_active=True).order_by(
            "name"
        )
...
def accessories_fast_sell(request):
    ...
    recent_sales = AccessoryStockLog.objects.filter(business=business, action="SALE").select_related(
        "product", "by_user"
    )
...
def accessories_lookup_api(request):
    ...
    if barcode:
        ...
        product = AccessoryProduct.objects.filter(business=business, barcode=normalized, is_active=True).first()

    # Fallback to search by name
    if not product and search:
        product = AccessoryProduct.objects.filter(business=business, name__icontains=search, is_active=True).first()

    if not product:
        return JsonResponse({"error": "Product not found"}, status=404)

    # Get stock for this product at current location
    stock = AccessoryStock.objects.filter(business=business, location=location, product=product).first()
...
def accessories_stock_in_api(request):
    ...
    try:
        ...
        validation = validate_selling_price(
            selling_price=selling_price, cost_price=order_price, product_name=name or "Accessory"
        )
...
def accessories_normal_sell(request):
    ...
    products = AccessoryProduct.objects.filter(business=business, is_active=True).order_by("category", "name")
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if not stock or stock.qty_on_hand < quantity:
                ...
                return JsonResponse(
                    {"error": f"Insufficient stock: requested {quantity}, available {available}"}, status=400
                )
