...
def stock_in(request):
    ...
    existing_brands = MerchProduct.objects.filter(
        business=business, kind=BusinessKind.CEMENT, is_active=True
    ...
    all_brands = seed_brands + [
        {"key": b.lower(), "name": b, "icon": "📦"} for b in brand_set if b.lower() not in ["akshar", "dangote"]
    ]
