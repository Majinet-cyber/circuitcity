...
def _get_phones_sales_trend(business, start_date, end_date, location, limit) -> Dict[str, Any]:
    ...
    product_sales = (
        sales_qs.values("item__product__brand", "item__product__model_name")
        .annotate(qty=Count("id"), revenue=Sum("price"))
        .order_by("-qty")[:limit]
    )
...
def _get_gym_sales_trend(business, start_date, end_date, location, limit) -> Dict[str, Any]:
    ...
    payment_sales = (
        payments_qs.values("member__membership_type")
        .annotate(qty=Count("id"), revenue=Sum("amount"))
        .order_by("-qty")[:limit]
    )
...
def _get_generic_sales_trend(business, start_date, end_date, location, limit) -> Dict[str, Any]:
    ...
    product_sales = (
        items_qs.values(product_field).annotate(qty=Count("id"), revenue=Sum("selling_price")).order_by("-qty")[:limit]
    )
