...
def get_inventory_kpis(
    *,
    ...
    # ================================================================
    revenue_agg = sales_qs.aggregate(revenue=Coalesce(Sum("price"), Value(0), output_field=dec2))
    ...
    cogs_agg = sales_qs.aggregate(
        cogs=Coalesce(Sum(Coalesce(F("item__order_price"), Value(0), output_field=dec2)), Value(0), output_field=dec2)
    )
    ...
    try:
        ...
        if business and start_date and end_date:
            ...
            costs_agg = period_costs_qs.aggregate(total=Coalesce(Sum("amount"), Value(0), output_field=dec2))
            ...
            commissions_agg = commissions_qs.aggregate(total=Coalesce(Sum("amount"), Value(0), output_field=dec2))
