...
def get_agent_earnings(
    ...
    try:
        ...
        wallets = AgentWallet.objects.filter(
    ...
    for row in agent_sales:
        ...
        results.append(AgentEarningRow(
            agent_id=agent_id,
    ...
    results.sort(
        key=lambda x: (-x.total_commission, -x.total_revenue, x.agent_name.lower())
    )
