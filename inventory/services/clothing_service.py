...
from inventory.models_verticals import ClothingSale, ClothingProductLog, ClothingProductAction, PaymentMethod, ClothingVariant
    ...
    *,
    ...
    if has_sizes or has_colors:
        ...
        try:
            ...
            variant_sku = generate_variant_sku(
                base_sku=product.internal_sku,
                size=variant_size,
                color=variant_color
            )
    ...
    *,
    ...
    if available_stock < quantity:
        raise ValidationError(
            f"Insufficient stock! Only {available_stock} available. Cannot sell {quantity} units."
        )
    ...
    message = (
        ...
        f"Revenue: K {total_price:,.2f} | Profit: K {profit:,.2f}"
    )
...
def get_top_sellers(business, location=None, days: int = 7, limit: int = 10) -> List[Dict]:
    ...
    top_products = sales_qs.values('product').annotate(
...
def get_slow_movers(business, location=None, days: int = 30, limit: int = 10) -> List[MerchProduct]:
    ...
    sold_product_ids = ClothingSale.objects.filter(
        business=business,
        sold_at__gte=since,
