...
def stock_in_pharmacy(
    *,
    ...
    if product_id:
        ...
        try:
            product = MerchProduct.objects.select_for_update().get(
                pk=product_id, business=business, kind=BusinessKind.PHARMACY, is_active=True
            )
    ...
    try:
        ...
        message = (
            f"✅ Stocked in: {quantity} {unit}(s) = {qty_base_units} base units — {product.name} (Batch: {batch_number})"
        )

    # Update product total stock (aggregate across all batches)
    total_stock = (
        PharmacyBatch.objects.filter(business=business, merch_product=product, is_archived=False).aggregate(
...
def sell_pharmacy(
    *,
    ...
    if batch_id:
        ...
        try:
            batch = PharmacyBatch.objects.select_for_update().get(pk=batch_id, business=business, is_archived=False)
        except PharmacyBatch.DoesNotExist:
        ...
        batch = (
            ...
            .filter(business=business, barcode=barcode_normalized, is_archived=False, quantity__gt=0)
        ...
        if not batch:
            product = MerchProduct.objects.filter(
                business=business, kind=BusinessKind.PHARMACY, barcode=barcode_normalized, is_active=True
            ).first()
            ...
            batch = (
                PharmacyBatch.objects.select_for_update()
                .filter(business=business, merch_product=product, is_archived=False, quantity__gt=0)
                .order_by("expiry_date", "batch_number")
                .first()
            )
        ...
        batch = (
            PharmacyBatch.objects.select_for_update()
            .filter(business=business, merch_product_id=product_id, is_archived=False, quantity__gt=0)
            .order_by("expiry_date", "batch_number")
            .first()
        )
    ...
    updated = PharmacyBatch.objects.filter(pk=batch.pk, quantity__gte=qty_base_units).update(
