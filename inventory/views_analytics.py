...
def analytics_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        try:
            staff_list = (
                Membership.objects.filter(business=business, is_active=True)
                .select_related("user")
    ...
    try:
        ...
        costs_qs = WalletTransaction.objects.filter(
            ...
            Q(
                ...
                effective_date__gte=date_params["start_date"],
                effective_date__lte=date_params["end_date"],
            )
            | Q(type=TxnType.COST_RECURRING, effective_from__lte=date_params["end_date"])
        )

        # Group by type
        cost_breakdown = costs_qs.values("type").annotate(total=Sum("amount")).order_by("-total")
