    ...
    if eval.inside:
        if sess.arrival_at is None:
            ...
            TimeEvent.objects.create(
                session=sess, kind=TimeEvent.Kind.ARRIVE, at=now, lat=lat, lon=lon, accuracy_m=accuracy_m
            )
        if sess.state != TimeSession.State.ONSITE:
            TimeEvent.objects.create(
                session=sess, kind=TimeEvent.Kind.ONSITE, at=now, lat=lat, lon=lon, accuracy_m=accuracy_m
            )
        ...
        if sess.arrival_at and not sess.departure_at:
            ...
            TimeEvent.objects.create(
                session=sess, kind=TimeEvent.Kind.DEPART, at=now, lat=lat, lon=lon, accuracy_m=accuracy_m
            )
        if sess.state != TimeSession.State.OFFSITE:
            TimeEvent.objects.create(
                session=sess, kind=TimeEvent.Kind.OFFSITE, at=now, lat=lat, lon=lon, accuracy_m=accuracy_m
            )
        sess.state = TimeSession.State.OFFSITE

    # Always log the ping
    TimeEvent.objects.create(session=sess, kind=TimeEvent.Kind.PING, at=now, lat=lat, lon=lon, accuracy_m=accuracy_m)

    sess.save(update_fields=["arrival_at", "departure_at", "work_s", "offsite_s", "state", "state_since", "updated_at"])
