...
def fast_sell_sell(request):
    ...
    if not selling_price or selling_price <= 0:
        ...
        if not selling_price_str:
            return JsonResponse({
                "ok": False,
                "error": "Selling price is required for this product"
            }, status=400)
        ...
        with transaction.atomic():
            ...
            if not location:
                ...
                if not location:
                    return JsonResponse({
                        "ok": False,
                        "error": "No active location found. Please set up a location first."
                    }, status=400)
...
def fast_sell_kpis(request):
    ...
    if range_type == 'today':
        ...
        today_stats = qs_today.aggregate(
            count=Count('id'),
            revenue=Sum('price')
        )
        ...
        return JsonResponse({
            "ok": True,
            "kpis": {
                "today_sales": today_stats['count'] or 0,
        ...
        return JsonResponse({
            "ok": True,
            "kpis": {
                "mtd_sales": mtd_stats['count'] or 0,
                "mtd_revenue": str(mtd_stats['revenue'] or 0)
            }
        })
        ...
        return JsonResponse({
            "ok": False,
            "error": "Invalid range type. Use 'today' or 'mtd'"
        }, status=400)
