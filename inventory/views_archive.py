...
from inventory.models import MerchProduct, InventoryItem, Location
...
def archive_flow_start(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if scope == "location" and not location_id:
            ...
            return render(
                request,
                "inventory/archive/step1_scope.html",
                {
                    "business": business,
                    "locations": locations,
                },
...
def archive_flow_summary(request: HttpRequest) -> HttpResponse:
    ...
    if location:
        ...
        product_ids_with_stock = (
            InventoryItem.objects.filter(
                business=business, current_location=location, is_active=True, archived_at__isnull=True
            )
        ...
        products_count = MerchProduct.objects.filter(business=business, is_archived=False).count()
    else:
        # Entire business - count all non-archived products
        products_count = MerchProduct.objects.filter(business=business, is_archived=False).count()

    # Stock items count (InventoryItem for phones)
    stock_items_count = InventoryItem.objects.filter(business=business, is_active=True, archived_at__isnull=True)
    if location:
        stock_items_count = stock_items_count.filter(current_location=location)
    stock_items_count = stock_items_count.count()

    # Barcodes count
    barcodes_count = InventoryBarcode.objects.filter(business=business, is_archived=False)
    if location:
        barcodes_count = barcodes_count.filter(location=location)
    barcodes_count = barcodes_count.count()

    # Laptop serials count
    laptop_serials_count = LaptopSerial.objects.filter(business=business, is_active=True, archived_at__isnull=True)
    if location:
    ...
    return render(
        request,
        "inventory/archive/step2_summary.html",
        {
            "business": business,
            "location": location,
            "scope": scope,
...
def archive_flow_confirm(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if confirmation_text != expected_text:
            ...
            return render(
                request,
                "inventory/archive/step3_confirm.html",
                {
                    "business": business,
                    "location": location,
                    "scope": scope,
...
def archive_flow_execute(request: HttpRequest) -> HttpResponse:
    ...
    if location:
        ...
        for product in products:
            ...
            has_barcodes = InventoryBarcode.objects.filter(
                product=product, location=location, is_archived=False
            ).exists()
        ...
        archived_counts["products"] = products.update(
            is_archived=True, archived_at=now, archived_by=request.user, is_active=False
        )
    ...
    archived_counts["stock_items"] = stock_items.update(archived_at=now, archived_by=request.user, is_active=False)
    ...
    archived_counts["barcodes"] = barcodes.update(is_archived=True, archived_at=now, archived_by=request.user)
    ...
    archived_counts["laptop_serials"] = laptop_serials.update(
        is_active=False, archived_at=now, archived_by=request.user
    )
    ...
    return render(
        request,
        "inventory/archive/step4_success.html",
        {
            "business": business,
            "location": location,
            "scope": scope,
