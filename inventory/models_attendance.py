...
from .models import Location  # your existing locations
...
class TimeLog(models.Model):
    ...
    """
    business = models.ForeignKey(Business, on_delete=models.CASCADE, related_name="time_logs", db_index=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="time_logs", db_index=True)
    location = models.ForeignKey(Location, on_delete=models.SET_NULL, null=True, blank=True, related_name="time_logs")
    kind = models.CharField(max_length=10, choices=CHECKIN_TYPES, default="ARRIVAL", db_index=True)
    ts = models.DateTimeField(default=timezone.now, db_index=True)

    # optional geo snapshot at check-in
...
# ---------- Policy (defaults; override per business later if you want) ----------
...
def compute_attendance_outcome(at_ts: datetime, kind: str) -> AttendanceOutcome:
    ...
    opening = local.replace(hour=DEFAULT_OPENING_HOUR.hour, minute=DEFAULT_OPENING_HOUR.minute, second=0, microsecond=0)
    ...
    if delta.total_seconds() > 0:
        mins = int((delta.total_seconds() + 59) // 60)  # ceil
        blocks = (mins + 29) // 30  # per started 30-min block
