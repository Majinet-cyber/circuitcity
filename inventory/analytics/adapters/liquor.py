...
class LiquorAdapter(AnalyticsAdapter):
    ...
    def get_sales_queryset(self, business, location=None) -> QuerySet:
        ...
        qs = LiquorSale.objects.filter(
            business=business,
            is_free=False
        ).select_related('product', 'shift', 'shift__location', 'sold_by')
    ...
    def charts(
        ...
        for pm in payment_mix:
            ...
            method_display = dict(LiquorSale._meta.get_field('payment_method').choices).get(method, method.replace('_', ' ').title())
    ...
    def vertical_sections(
        ...
        # Credit vs cash mix
        credit_sales = sales_qs.filter(is_credit=True).aggregate(
            total=Coalesce(Sum('total_price'), Decimal('0.00'))
        )['total'] or Decimal('0.00')
        
        cash_sales = sales_qs.filter(is_credit=False).aggregate(
            total=Coalesce(Sum('total_price'), Decimal('0.00'))
        )['total'] or Decimal('0.00')
