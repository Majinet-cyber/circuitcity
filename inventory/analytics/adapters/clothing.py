# inventory/analytics/adapters/clothing.py
"""
Analytics adapter for clothing vertical.
"""
...
class ClothingAdapter(AnalyticsAdapter):
    """Analytics adapter for clothing vertical."""
    
    def get_sales_queryset(self, business, location=None) -> QuerySet:
        """Get base sales queryset."""
        qs = ClothingSale.objects.filter(business=business).select_related('product', 'sold_by')
        # ClothingSale doesn't have location field, so location filter is ignored
        return qs
    
    def get_stock_queryset(self, business, location=None) -> QuerySet:
        """Get base stock queryset."""
        qs = MerchProduct.objects.filter(business=business, kind='clothing', is_active=True)
    ...
    def get_search_fields(self) -> List[str]:
        """Return search fields for clothing."""
        return ['product__name', 'notes', 'sold_by__username']
    ...
    def kpis(
        ...
        cost_of_goods = sales_qs.aggregate(total=Coalesce(Sum('total_cost'), Decimal('0.00')))['total'] or Decimal('0.00')
    ...
    def charts(
        ...
        """Generate chart data for clothing."""
        sales_qs = self.get_sales_queryset(business, location)
        start_dt = timezone.make_aware(timezone.datetime.combine(start_date, timezone.datetime.min.time()))
        end_dt = timezone.make_aware(timezone.datetime.combine(end_date + timedelta(days=1), timezone.datetime.min.time()))
        sales_qs = sales_qs.filter(sold_at__gte=start_dt, sold_at__lt=end_dt)
        ...
        # Sales trend
        daily_sales = sales_qs.annotate(day=TruncDate('sold_at')).values('day').annotate(
        ...
        for pm in payment_mix:
            ...
            method_display = dict(ClothingSale._meta.get_field('payment_method').choices).get(method, method.replace('_', ' ').title())
        ...
        # Top products
        top_products = sales_qs.values('product__name', 'product_id').annotate(
            revenue=Coalesce(Sum('total_price'), Decimal('0.00')),
        ...
        # Top agents/cashiers
        top_agents = sales_qs.filter(sold_by__isnull=False).values(
            'sold_by__id', 'sold_by__first_name', 'sold_by__last_name', 'sold_by__username'
        ).annotate(
            revenue=Coalesce(Sum('total_price'), Decimal('0.00')),
            units_sold=Sum('quantity')
        ).order_by('-revenue')[:10]
        ...
        top_agents_data = [
            {
                'name': f"{item['sold_by__first_name'] or ''} {item['sold_by__last_name'] or ''}".strip() or item['sold_by__username'] or 'Unknown',
                'revenue': float(item['revenue']),
                'units_sold': item['units_sold'] or 0,
            }
        ...
        # For clothing, we'd need to calculate stock value from MerchProduct
        # This is simplified - you may need to adjust based on your actual stock tracking
        stock_count = stock_qs.count()
        
        stock_overview = {
            'stock_value': 0.0,  # Would need to calculate from stock quantities
    ...
    def vertical_sections(
        ...
        """Vertical-specific sections for clothing."""
        sales_qs = self.get_sales_queryset(business, location)
        start_dt = timezone.make_aware(timezone.datetime.combine(start_date, timezone.datetime.min.time()))
        end_dt = timezone.make_aware(timezone.datetime.combine(end_date + timedelta(days=1), timezone.datetime.min.time()))
        sales_qs = sales_qs.filter(sold_at__gte=start_dt, sold_at__lt=end_dt)
        
        # Category mix (if category field exists on product)
        category_mix = sales_qs.values('product__category').annotate(
            count=Count('id'),
            revenue=Coalesce(Sum('total_price'), Decimal('0.00'))
        ).order_by('-revenue')[:5]
