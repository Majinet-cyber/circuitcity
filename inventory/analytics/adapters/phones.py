...
class PhonesAdapter(AnalyticsAdapter):
    ...
    def get_sales_queryset(self, business, location=None) -> QuerySet:
        ...
        qs = InventoryItem.objects.filter(business=business, status="SOLD", sold_at__isnull=False).select_related(
    ...
    def get_stock_queryset(self, business, location=None) -> QuerySet:
        ...
        qs = InventoryItem.objects.filter(business=business, status="IN_STOCK", is_active=True).select_related(
    ...
    def kpis(
        ...
        # Calculate KPIs
        revenue = sales_qs.aggregate(total=Coalesce(Sum("selling_price"), Decimal("0.00")))["total"] or Decimal("0.00")

        cost_of_goods = sales_qs.aggregate(total=Coalesce(Sum("order_price"), Decimal("0.00")))["total"] or Decimal(
            "0.00"
        )
    ...
    def charts(
        ...
        payment_mix = (
            ...
            .annotate(total=Coalesce(Sum("selling_price"), Decimal("0.00")), count=Count("id"))
            .order_by("-total")
        )
        ...
        top_products = (
            sales_qs.annotate(
                model_label=CoalesceFunc(
                    Concat(F("product__brand"), Value(" "), F("product__model"), output_field=CharField()),
                    F("product__model"),
                    F("product__name"),
                    Value("Unknown"),
            ...
            .values("model_label")
            .annotate(revenue=Coalesce(Sum("selling_price"), Decimal("0.00")), units_sold=Count("id"))
            .order_by("-revenue")[:10]
        )
        ...
        # Top agents
        top_agents = (
            sales_qs.filter(assigned_agent__isnull=False)
            .values(
                "assigned_agent__id",
        ...
        top_agents_data = [
            {
                "name": f"{item['assigned_agent__first_name'] or ''} {item['assigned_agent__last_name'] or ''}".strip()
                or item["assigned_agent__username"]
                or "Unknown",
                "revenue": float(item["revenue"]),
                "units_sold": item["units_sold"],
            }
    ...
    def vertical_sections(
        ...
        # Top brands
        top_brands = (
            sales_qs.values("product__brand")
            .annotate(count=Count("id"), revenue=Coalesce(Sum("selling_price"), Decimal("0.00")))
            .order_by("-revenue")[:5]
        )
