...
class GymAdapter(AnalyticsAdapter):
    ...
    def kpis(
        ...
        active_memberships = members_qs.filter(
        ...
        today_end = timezone.make_aware(timezone.datetime.combine(today + timedelta(days=1), timezone.datetime.min.time()))
        new_members_today = members_qs.filter(
        ...
        new_members_this_month = members_qs.filter(
            joined_at__gte=month_start,
            joined_at__lt=today_end
        ).count()
        
        # Payments today
        payments_today = payments_qs.filter(
        ...
        # Revenue this month
        revenue_this_month = payments_qs.filter(
            paid_at__gte=month_start,
            paid_at__lt=today_end
        ).aggregate(total=Coalesce(Sum('amount'), Decimal('0.00')))['total'] or Decimal('0.00')
        ...
        churned = 0
        ended_memberships = members_qs.filter(
            membership_end__gte=start_date,
            membership_end__lte=end_date
        )
    ...
    def charts(
        ...
        daily_payments = payments_qs.annotate(day=TruncDate('paid_at')).values('day').annotate(
        ...
        # Payment mix
        payment_mix = payments_qs.values('payment_method').annotate(
        ...
        for pm in payment_mix:
            ...
            method_display = dict(GymPayment._meta.get_field('payment_method').choices).get(method, method.replace('_', ' ').title())
        ...
        # Top trainers
        top_agents = payments_qs.filter(trainer__isnull=False).values(
            'trainer__id', 'trainer__name'
        ).annotate(
            revenue=Coalesce(Sum('amount'), Decimal('0.00')),
            count=Count('id')
        ).order_by('-revenue')[:10]
    ...
    def vertical_sections(
        ...
        new_members = members_qs.filter(
            joined_at__gte=start_dt,
            joined_at__lt=end_dt
        ).count()
        
        # New payments in period
        new_payments = payments_qs.filter(
