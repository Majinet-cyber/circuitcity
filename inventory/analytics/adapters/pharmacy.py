...
class PharmacyAdapter(AnalyticsAdapter):
    ...
    def get_sales_queryset(self, business, location=None) -> QuerySet:
        ...
        qs = PharmacySale.objects.filter(
            ...
            is_deleted=False,
            is_reversed=False
        ).select_related('batch', 'batch__merch_product', 'sold_by')
    ...
    def charts(
        ...
        costs_annotation = ExpressionWrapper(F('unit_cost') * F('quantity'), output_field=DecimalField(max_digits=12, decimal_places=2))
        daily_sales_with_cost = sales_qs.annotate(day=TruncDate('sold_at'), sale_cost=costs_annotation).values('day').annotate(
            revenue=Coalesce(Sum('total_amount'), Decimal('0.00')),
        ...
        # Top products
        top_products = sales_qs.values('batch__merch_product__name', 'batch__merch_product_id').annotate(
            revenue=Coalesce(Sum('total_amount'), Decimal('0.00')),
        ...
        stock_value = stock_qs.aggregate(
            total=Coalesce(Sum('unit_cost') * F('quantity_remaining'), Decimal('0.00'))
        )['total'] or Decimal('0.00')
    ...
    def vertical_sections(
        ...
        expiring_soon = batches_qs.filter(
            expiry_date__gte=today,
            expiry_date__lte=today + timedelta(days=30)
        ).count()
        
        # Stock aging (group by days until expiry)
        aging_batches = batches_qs.filter(expiry_date__isnull=False).values('expiry_date').annotate(
            count=Count('id')
