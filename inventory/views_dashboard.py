...
def inventory_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        ).order_by('stock_count')[:10]  # Top 10 low/out of stock
        
        for product in products_with_stock:
            low_stock_items.append({
                'id': product.id,
                'name': f"{product.brand or ''} {product.model or ''} {product.variant or ''}".strip() or 'Unknown Product',
    ...
    try:
        ...
        top_products_query = (
            sales_qs
            .values('item__product__id', 'item__product__brand', 'item__product__model', 'item__product__variant')
            .annotate(
                units_sold=Count('id'),
                revenue=Coalesce(Sum('price'), Decimal('0.00'), output_field=DecimalField())
            )
