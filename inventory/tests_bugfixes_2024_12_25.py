...
from inventory.models_verticals import LiquorSale, CementSale, GrocerySale, CementCost
...
class ManagerRollbackPermissionTest(TestCase):
    ...
    def setUp(self):
        ...
        self.manager = User.objects.create_user(username="manager", email="manager@test.com", password="test123")
        self.agent = User.objects.create_user(username="agent", email="agent@test.com", password="test123")

        Membership.objects.create(user=self.manager, business=self.business, role="MANAGER", status="ACTIVE")
        self.location = Location.objects.create(business=self.business, name="Store")

        Membership.objects.create(user=self.agent, business=self.business, role="AGENT", location=self.location)
        ...
        product = Product.objects.create(
            code=f"TEST-{uuid.uuid4().hex[:8]}", name="Test Phone", brand="Test", model="Test Model"
        )
        item = InventoryItem.objects.create(business=self.business, product=product, imei="123456789012345")
...
class CementVerticalTest(TestCase):
    ...
    def setUp(self):
        self.business = Business.objects.create(name="Test Cement Store", business_kind=BusinessKind.CEMENT)
        self.user = User.objects.create_user(username="testuser", password="test123")
        Membership.objects.create(user=self.user, business=self.business, role="MANAGER")
...
class GroceriesVerticalTest(TestCase):
    ...
    def setUp(self):
        self.business = Business.objects.create(name="Test Grocery Store", business_kind=BusinessKind.GROCERY)
        self.user = User.objects.create_user(username="testuser", password="test123")
        Membership.objects.create(user=self.user, business=self.business, role="MANAGER")
    ...
    def test_product_add_from_seed(self):
        ...
        product = MerchProduct.objects.get(business=self.business, name="Sugar Packet 1kg", kind=BusinessKind.GROCERY)
    ...
    def test_product_add_custom(self):
        ...
        # Verify product was created
        product = MerchProduct.objects.get(business=self.business, name="Custom Item", kind=BusinessKind.GROCERY)
...
class MenuIsolationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.grocery_business, _ = Business.objects.get_or_create(
            ...
            defaults={"name": f"Grocery Store Test {unique_id}", "business_kind": BusinessKind.GROCERY},
        )
        self.cement_business, _ = Business.objects.get_or_create(
            slug=f"cement-test-{unique_id}",
            defaults={"name": f"Cement Store Test {unique_id}", "business_kind": BusinessKind.CEMENT},
        )
        self.phone_business, _ = Business.objects.get_or_create(
            slug=f"phone-test-{unique_id}",
            defaults={"name": f"Phone Store Test {unique_id}", "business_kind": BusinessKind.PHONES},
        )
