...
def validate_cider_pack_size(pack_size: int) -> None:
    ...
    if pack_size != 6:
        raise ValidationError(
            f"Cider pack size must be exactly 6 (6-pack). " f"Got: {pack_size}. Cider does not use crates."
        )
...
def validate_unit_for_kind(liquor_kind: str, unit: str, pack_enabled: bool = False, for_stock_in: bool = False) -> None:
    ...
    if unit_lower not in allowed_units:
        ...
        if unit_lower == "shot" and liquor_kind in (LiquorKind.BEER, LiquorKind.CIDER):
            raise ValidationError(f"{liquor_kind.title()} cannot be sold by shot. " f"Use bottle/can or pack.")
        elif unit_lower == "shot" and liquor_kind == LiquorKind.WINE:
            raise ValidationError(f"Wine cannot be sold by shot. " f"Use glass or bottle.")
        elif unit_lower == "crate" and liquor_kind == LiquorKind.CIDER:
            raise ValidationError(f"Cider uses 6-packs, not crates. " f"Please select 6-pack or bottle/can.")
            ...
            raise ValidationError(
                f"{liquor_kind.title()} stock-in must be by BOTTLE only. " f"Cannot stock-in by shot. Use bottle."
            )
        else:
            context = "stock-in" if for_stock_in else "selling"
            raise ValidationError(
                f"Invalid unit '{unit}' for {liquor_kind.title()} {context}. " f"Allowed units: {allowed_str}"
            )
...
def to_base_units(
    ...
    if unit_lower in pack_unit_aliases:
        ...
        if liquor_kind == LiquorKind.CIDER and unit_lower == "crate":
            raise ValidationError(f"Cider uses 6-packs, not crates. Please select 6-pack or bottle/can.")
        ...
        if pack_label_lower == unit_normalized or unit_lower in ["crate", "case", "pack"]:
            ...
            raise ValidationError(
                f"This product uses '{pack_label}' not '{unit}'. " f"Please select '{pack_label}' or base unit."
            )

    # Unknown unit
    raise ValidationError(
        f"Cannot convert '{unit}' to base units. " f"Expected base unit '{base_unit}' or configured pack unit."
    )
...
def format_display_quantity(
    qty_base: int, base_unit: str, pack_size: Optional[int] = None, pack_label: Optional[str] = None
) -> str:
    """
