    ...
    try:
        ...
        if category == LiquorKind.BEER:
            ...
            if enable_glass:
                ...
                product_data["glasses_per_bottle"] = int(
                    details.get("glasses_per_bottle", defaults["glasses_per_bottle"])
                )
                ...
                if sell_per_glass:
                    ...
                    product_data["price_per_glass"] = (
                        sell_per_bottle / Decimal(product_data["glasses_per_bottle"])
            ...
            if enable_shot:
                ...
                if sell_per_shot:
                    ...
                    product_data["price_per_shot"] = (sell_per_bottle / Decimal(sellable_shots)).quantize(
                        Decimal("0.01")
                    )
        ...
        if isinstance(e, IntegrityError):
            ...
            return JsonResponse(
                {"success": False, "error": "Database error. Please check your input and try again."}, status=400
            )
        elif isinstance(e, ValidationError):
            return JsonResponse({"success": False, "error": str(e)}, status=400)
        else:
            return JsonResponse({"success": False, "error": f"Failed to create product: {str(e)}"}, status=500)
