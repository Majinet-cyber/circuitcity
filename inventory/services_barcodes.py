...
def validate_barcodes_for_qty(
    ...
    existing = InventoryBarcode.objects.filter(
        business=business,
        code__in=codes,
        is_archived=False
...
def archive_barcodes_for_product(product, user=None):
    ...
    barcodes = InventoryBarcode.objects.filter(
        product=product,
        is_archived=False
    )
    
    count = barcodes.update(
        is_archived=True,
