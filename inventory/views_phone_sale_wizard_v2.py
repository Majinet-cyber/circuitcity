...
def _step1_imei(request, business, wizard_data):
    ...
    if request.method == "POST":
        ...
        if not imei:
            ...
            if not stock_item:
                messages.error(
                    request,
                    f"IMEI {imei} is not in stock at your location. " f"Please check the number or ask your manager.",
                )
            else:
                ...
                messages.success(request, f"✅ In stock – {wizard_data['product_name']} at {wizard_data['location']}")
    ...
    return render(
        request,
        "inventory/phone_sale_wizard_v2_step1.html",
        {
            "step": 1,
            "business": business,
...
def _step2_price(request, business, wizard_data):
    ...
    try:
        stock_item = InventoryItem.objects.select_related("product").get(id=wizard_data["stock_id"], business=business)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if selling_price <= 0:
                ...
                return render(
                    request,
                    "inventory/phone_sale_wizard_v2_step2.html",
                    {
                        "step": 2,
                        "business": business,
                        "wizard_data": wizard_data,
...
def _step3_payment(request, business, wizard_data):
    ...
    if request.method == "POST":
        ...
        if payment_method not in valid_methods:
            ...
            return render(
                request,
                "inventory/phone_sale_wizard_v2_step3.html",
                {
                    "step": 3,
                    "business": business,
                    "wizard_data": wizard_data,
...
def _complete_sale(request, business, wizard_data, payment_method):
    ...
    try:
        ...
        try:
            stock_item = InventoryItem.objects.select_for_update().get(id=stock_id, business=business)
        except InventoryItem.DoesNotExist:
        ...
        return {
            ...
            "profit": profit,
            "payment_method": payment_method,
            "payment_method_display": dict(Sale._meta.get_field("payment_method").choices).get(
                payment_method, payment_method
            ),
        }
