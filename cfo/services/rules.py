...
def ensure_alert(kind, severity, subject_type, subject_id, message):
    return Alert.objects.create(
        kind=kind, severity=severity, subject_type=subject_type, subject_id=str(subject_id), message=message
    )
...
def run_rules():
    ...
    for row in exp:
        ...
        if b and row["total"] > b.limit_amount:
            ensure_alert(
                ...
                "branch",
                row["branch"],
                f"Budget overshoot for category {row['category']} by {row['total'] - b.limit_amount}",
            )
    ...
    avg_14 = (
        Expense.objects.filter(date__gte=last_14, date__lt=today).aggregate(s=Sum("amount"))["s"] or Decimal("0")
    ) / Decimal("14")
    ...
    commits = CashLedger.objects.filter(entry_type="commit", ref_type="salary", date__gte=month_start).aggregate(
        s=Sum("amount")
    )["s"] or Decimal("0")
    ...
    if opening < commits:
        ensure_alert(
            ...
            "global",
            "0",
            f"Projected opening balance < payroll commitments: {opening} < {commits}",
        )
