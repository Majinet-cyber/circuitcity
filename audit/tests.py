...
class AuditLogModelTest(TestCase):
    ...
    def test_audit_log_ordering(self):
        ...
        old_log = AuditLog.objects.create(business=self.business, user=self.user, action="OLD_ACTION", entity="Test")

        # Create a newer log
        new_log = AuditLog.objects.create(business=self.business, user=self.user, action="NEW_ACTION", entity="Test")
...
class AuditUtilsTest(TestCase):
    ...
    def test_log_hq_action_handles_no_business(self):
        ...
        log = log_hq_action(request, action="TEST_ACTION", entity_type="TEST", message="Test with no business")
...
class AuditLogViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.staff_user = User.objects.create_user(username="hq_staff", password="testpass123", is_staff=True)
        self.normal_user = User.objects.create_user(username="normal_user", password="testpass123")
        self.client = Client()
    ...
    def test_audit_log_list_shows_staff_activity_only(self):
        ...
        staff_log = AuditLog.objects.create(
            business=self.business, user=self.staff_user, action="VIEW_PAGE", entity="Test", message="Staff action"
        )
    ...
    def test_audit_log_date_filtering(self):
        ...
        old_log = AuditLog.objects.create(
            business=self.business, user=self.staff_user, action="OLD_ACTION", entity="Test"
        )
        old_log.created_at = last_week
        old_log.save()

        new_log = AuditLog.objects.create(
            business=self.business, user=self.staff_user, action="NEW_ACTION", entity="Test"
        )
        ...
        response = self.client.get(
            ...
            {"start_date": yesterday.strftime("%Y-%m-%d"), "end_date": now.strftime("%Y-%m-%d")},
        )
    ...
    def test_audit_log_action_filtering(self):
        ...
        view_log = AuditLog.objects.create(
            business=self.business, user=self.staff_user, action="VIEW_PAGE", entity="Test"
        )

        export_log = AuditLog.objects.create(
            business=self.business, user=self.staff_user, action="EXPORT_CSV", entity="Test"
        )

        self.client.force_login(self.staff_user)

        # Filter by action
        response = self.client.get(reverse("audit:log_list"), {"action": "EXPORT"})
    ...
    def test_audit_log_business_filtering(self):
        ...
        log1 = AuditLog.objects.create(business=self.business, user=self.staff_user, action="TEST", entity="Test")

        log2 = AuditLog.objects.create(business=business2, user=self.staff_user, action="TEST", entity="Test")

        self.client.force_login(self.staff_user)

        # Filter by business
        response = self.client.get(reverse("audit:log_list"), {"business": self.business.pk})
    ...
    def test_audit_log_csv_export(self):
        ...
        # Request CSV export
        response = self.client.get(reverse("audit:log_list"), {"export": "csv"})
    ...
    def test_audit_log_pagination(self):
        ...
        # Create many logs
        for i in range(60):
            AuditLog.objects.create(business=self.business, user=self.staff_user, action=f"ACTION_{i}", entity="Test")
