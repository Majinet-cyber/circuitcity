    ...
    def reset_business_data(self, request, queryset):
        ...
        for business in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f"Failed to reset {business.name}: {str(e)}",
        ...
        if count > 0:
            self.message_user(
                request,
                f"Successfully reset {count} business(es).",
...
class MembershipAdmin(admin.ModelAdmin):
    ...
    def delete_view(self, request, object_id, extra_context=None):
        ...
        try:
            ...
            self.message_user(
                request,
                f"Deletion failed: {str(e)}. Use 'Deactivate membership' instead.",
                level=messages.ERROR
            )
            return HttpResponseRedirect(reverse("admin:tenants_membership_changelist"))
    ...
    def hard_delete_memberships(self, request, queryset):
        ...
        if count > 0:
            self.message_user(
                request,
                f"Hard deleted {count} membership(s).",
                level=messages.SUCCESS
            )
        if errors:
