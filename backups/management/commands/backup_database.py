...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cmd = [
            ...
            str(output_path),
            "--format",
            "custom",  # Custom format for faster restore
            "--verbose",
        ]
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Backup created successfully: {output_path} ({file_size_mb:.2f} MB)")
            )
    ...
    def _upload_to_s3(self, backup_path: Path):
        ...
        try:
            ...
            if not s3_bucket:
                self.stdout.write(self.style.WARNING("BACKUP_S3_BUCKET not configured, skipping S3 upload"))
                return
            ...
            self.stdout.write(self.style.SUCCESS(f"✓ Uploaded to S3: s3://{s3_bucket}/{s3_key}"))
    ...
    def _log_backup(self, backup_path: Path, file_size: int):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f"Could not log backup: {e}"))
