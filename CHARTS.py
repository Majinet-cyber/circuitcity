...
creds = ServiceAccountCredentials.from_json_keyfile_name(
    "circuit-city-dashboard-xxx.json", scope  # replace with your actual file name
)
...
def dashboard():
    ...
    cashflow_df = pd.DataFrame(
        {"Date": dates[: len(savings)], "Savings": savings, "Total Savings": totals[: len(savings)]}
    )

    # âœ… Make Plotly chart
    fig = px.bar(cashflow_df, x="Date", y=["Savings", "Total Savings"], barmode="group", title="Live Cash Reserve")
