    ...
    def test_hq_command_center_includes_mobile_css(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.assertTrue(
                    has_mobile_css or has_hq_page_class,
                    "HQ command center should include mobile-first support"
                )
        except Exception:
    ...
    def test_base_hq_includes_viewport_meta(self):
        ...
        if response.status_code == 200:
            self.assertContains(
                response,
                'name="viewport"',
                msg_prefix='HQ pages should have viewport meta tag'
            )
        ...
        if os.path.exists(css_path):
            ...
            for selector in required_selectors:
                self.assertIn(
                    selector,
                    css_content,
                    f"hq-mobile.css should contain {selector}"
                )
